...
Client = (function(_super) {
  ...
  Client.prototype._reset = function() {
    ...
    return this.preGameEventsBuffer = [];
  };
  ...
  function Client(adapter, gameEventFactory, clientMessageFactory, simulationEventFactory) {
    ...
    this.adapter.on('ClientAdapter::Disconnected', (function(_this) {
      return function() {
        ...
        if (_this._disconnectCallback) {
          callback = _this._disconnectCallback;
          _this._disconnectCallback = null;
          return callback();
        } else {
    ...
    this.adapter.on('ClientAdapter::Packet', (function(_this) {
      return function(data) {
        ...
        _this._debugIncomingMessage(msg);
        switch (msg.type) {
  ...
  Client.prototype._sendMessage = function(msg) {
    ...
    this._debugOutgoingMessage(msg);
  ...
  Client.prototype._debugIncomingMessage = function(msg) {
    return this.emit('incomingMessage', msg);
  };

  Client.prototype._debugOutgoingMessage = function(msg) {
    return this.emit('outgoingMessage', msg);
  };
...
var adapterConstructors, addAdapterConstructor, createAdapter, createSimulation, getAdapterConstructor;
...
createSimulation = function(opts) {
  ...
  if (callback1 = opts.adapter.spyOnDataIn) {
    adapter.on('ClientAdapter::Packet', function(data) {
      return callback1(simulation, data);
    });
  }
  if (callback2 = opts.adapter.spyOnDataOut) {
    adapter.on('ClientAdapter::SendingData', function(data) {
      return callback2(simulation, data);
    });
  }
  if (opts.client) {
    if (callback3 = opts.client.spyOnIncoming) {
      client.on('incomingMessage', function(msg) {
        return callback3(simulation, msg);
      });
    }
    if (callback4 = opts.client.spyOnOutgoing) {
      client.on('outgoingMessage', function(msg) {
        return callback4(simulation, msg);
      });
    }
  }
  return simulation;
...
addAdapterConstructor = function(type, constructor) {
  return adapterConstructors[type] = constructor;
};

getAdapterConstructor = function(type) {
  return adapterConstructors[type];
};

createAdapter = function(adapterOpts) {
  var constructor;
  if (constructor = adapterConstructors[adapterOpts.type]) {
    return constructor(adapterOpts.options);
  } else {
    throw new Error("No adapter constructor found for type '" + adapterOpts.type + "'");
  }
};

addAdapterConstructor('socket_io', function(opts) {
  var SocketIOClientAdapter;
  SocketIOClientAdapter = require('./socket_io_client_adapter.coffee');
  return new SocketIOClientAdapter(opts);
});
...
},{"./client.coffee":2,"./client_message_factory.coffee":3,"./event_emitter.coffee":4,"./fix_float.coffee":5,"./game_event_factory.coffee":6,"./simulation.coffee":8,"./simulation_event_factory.coffee":9,"./simulation_state_factory.coffee":11,"./simulation_state_serializer.coffee":12,"./socket_io_client_adapter.coffee":13,"./turn_calculator.coffee":14,"./user_event_serializer.coffee":15,"./world_base.coffee":16}],8:[function(require,module,exports){
...
Simulation = (function() {
  Simulation.prototype._reset = function() {
    this.lastTurnTime = 0;
    this.currentTurnNumber = null;
    return this.simState = null;
  };

  function Simulation(_world, client, turnCalculator, simulationStateFactory, simulationStateSerializer, userEventSerializer) {
    this._world = _world;
    this.client = client;
  ...
  Simulation.prototype.stop = function() {
    return this.client.disconnect((function(_this) {
      return function() {
        return _this._beDisconnected();
      };
    })(this));
  };
  ...
  Simulation.prototype.getWorldIntrospector = function() {
    return this._world.getIntrospector();
  };
  ...
  Simulation.prototype.update = function(timeInSeconds) {
    ...
    if (this.simState) {
      ...
      this.turnCalculator.stepUntilTurnTime(this.simState, this._world, elapsedTurnTime);
    }
      ...
      return function(gameEvent) {
        ...
        switch (gameEvent.type) {
          case 'GameEvent::TurnComplete':
            ...
            _this.turnCalculator.advanceTurn(_this.simState, _this._world);
              ...
              switch (simEvent.type) {
                case 'SimulationEvent::Event':
                  ...
                  if (userEvent.type === 'UserEvent::WorldProxyEvent') {
                    if (_this._world[userEvent.method]) {
                      (_ref1 = _this._world)[userEvent.method].apply(_ref1, [simEvent.playerId].concat(__slice.call(userEvent.args)));
                    } else {
                    ...
                    _this._world.incomingEvent(simEvent.playerId, userEvent);
                  }
                  break;
                case 'SimulationEvent::PlayerJoined':
                  _this._world.playerJoined(simEvent.playerId);
                  break;
                case 'SimulationEvent::PlayerLeft':
                  _this._world.playerLeft(simEvent.playerId);
              }
            }
            return gameEvent.checksumClosure(_this._world.getChecksum());
          case 'GameEvent::StartGame':
            _this._debug('GameEvent::StartGame', gameEvent);
            ...
            _this._world.setData(gameEvent.worldState);
          ...
          case 'GameEvent::GamestateRequest':
            _this._debug('GameEvent::GameStateRequest', gameEvent);
            ...
            return gameEvent.gamestateClosure(packedSimState, _this._world.getData());
          case 'GameEvent::Disconnected':
            _this._debug('GameEvent::Disconnected', gameEvent);
            return _this._beDisconnected();
        }
      };
    })(this));
  ...
  Simulation.prototype._beDisconnected = function() {
    this._reset();
    return this._world.theEnd();
  };
...
SocketIOClientAdapter = (function(_super) {
  ...
  function SocketIOClientAdapter(opts) {
    if (opts == null) {
      opts = {};
    }
    this._debugOn = false;
    this.url = opts.url || (function() {
      throw new Error("SocketIOClientAdapter needs a 'url' argument");
    })();
    this.connectionBuilder = opts.connectionBuilder || window.io || (function() {
      throw new Error("SocketIOClientAdapter requires a connectionBuilder (typically Socket.io's window.io object) but it wasn't provided explicitly, and window.io isn't set.  Help?");
    })();
    this._first = true;
  }

  SocketIOClientAdapter.prototype.connect = function() {
    if (this._first) {
      this._debug("connecting");
      this._first = false;
      this.socket = this.connectionBuilder.connect(this.url);
    } else {
      this._debug("REconnecting");
      this.socket = this.connectionBuilder.connect(null, {
        'force new connection': true
      });
    }
    this.socket.on('connection', (function(_this) {
      return function(data) {
        _this._debug("connection established");
        return _this.emiy('ClientAdapter::Connected');
      };
    })(this));
    this.socket.on('data', (function(_this) {
      return function(data) {
        _this._debug("got data", data);
        return _this.emit('ClientAdapter::Packet', data);
      };
    })(this));
    return this.socket.on('disconnect', (function(_this) {
      return function() {
        _this._debug("disconnected");
        _this.emit('ClientAdapter::Disconnected');
  ...
  SocketIOClientAdapter.prototype.send = function(data) {
    if (this.socket) {
      ...
      this.emit('ClientAdapter::SendingData', data);
  ...
  SocketIOClientAdapter.prototype._debug = function() {
    ...
    args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    if (this._debugOn) {
      return console.log("[SocketIOClientAdapter]", args);
    }
  };
...
WorldBase = (function() {
  ...
  WorldBase.prototype.getIntrospector = function() {
    throw new Error("Please implemenet WorldBase#getIntrospector");
  };

  WorldBase.prototype.theEnd = function() {
    return console.log("SimSim WorldBase: theEnd.  Override this method in your World class to silence this message and handle this event.");
  };
