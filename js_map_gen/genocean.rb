...
class GenOcean
	...
	def initialize(map, length)
		...
		#copies tempmap back into regular map
		for i in (0..@length-1)
			for j in (0..@length-1)
	...
	#check to see if the node is closed.  pushes the node to an array
	def checkNode(x, y)
	...
	#this makes sure we don't go out of bounds on our arrays
