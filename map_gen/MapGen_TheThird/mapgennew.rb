...
class MapGen
	...
	def initialize (window, width, height, rows)
		...
		@map_struct = Struct.new :elevation, :special, :type
		puts "initializing array..."
		@map = Array.new(@rows) { Array.new(@rows) { @map_struct.new -1, "" } }
		@tile_size = 10 #91312#48482#
		seed = 7623#rand(99999)
		@bitmap_name = seed
		puts "seed = " + seed.to_s
		...
		#puts Time.now.to_i
		puts "converting"
		c = Converter.new(@map, @rows)
		c.convert
		
		puts "saving file old..."
		saveFile(seed)
		puts "saving file new..."
		c.saveFileNew(seed)
		...
		for i in (0..@rows-1)
			for j in (0..@rows-1)
				if @map[i][j].type == "Ocean"
					...
					bmp[i,j] = "ff0000"
				elsif @map[i][j].type.include? "cliff"
					bmp[i,j] = "0000ff"
